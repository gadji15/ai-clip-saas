    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Link href={`/${locale}`} className={buttonStyles({ variant: 'ghost', size: 'sm' })}>
            {t('back')}
          </Link>
          <h1 className="text-2xl font-semibold tracking-tight">{t('title', { id })}</h1>
        </div>
          ...
          <CardContent>
            ...
            <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="text-sm font-medium text-slate-800">{t('progress.title')}</div>
                <div className="text-sm text-slate-600">{t('progress.value', { value: processing })}</div>
              </div>
              <div className="mt-3">
