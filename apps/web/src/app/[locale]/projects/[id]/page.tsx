...
export default async function ProjectDetailsPage({
  ...
  const status: ProjectStatus = id.includes('failed') ? 'failed' : id.includes('1') ? 'completed' : 'processing';
  const stage: Stage = status === 'completed' ? 'done' : status === 'failed' ? 'render' : 'transcribe';
  const progress = status === 'completed' ? 100 : status === 'failed' ? 72 : 42;

  const stageLabel = t(`stages.${stage}`);

  const clips = [
    { id: 'clip_a1', title: 'Hook + intro', score: 0.86, status: 'ready' as const },
    { id: 'clip_b2', title: 'Key insight', score: 0.77, status: 'ready' as const },
    { id: 'clip_c3', title: 'Closing', score: 0.41, status: 'pending' as const },
  ];
    ...
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <StatusBadge status={status} labels={{
          ...
          <CardContent>
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-xs font-medium text-slate-500">{t('overview.stage')}</div>
                  <div className="mt-1 text-sm font-semibold text-slate-900">{stageLabel}</div>
                </div>
                <div className="text-right">
                  <div className="text-xs font-medium text-slate-500">{t('overview.progress')}</div>
              ...
              <div className="space-y-3">
                {events.map((e, idx) => (
                  <div key={idx} className="flex gap-3">
                    <div className="w-14 shrink-0 text-xs font-medium text-slate-500">{e.ts}</div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-xs font-medium text-slate-600">{Math.round(c.score * 100)}%</div>
                      </div>
                    </div>
                  </Link>
                ))}
