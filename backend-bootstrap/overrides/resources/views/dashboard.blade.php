<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('projects.index') }}" class="text-sm text-indigo-600 hover:text-indigo-900">Projects</a>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('projects.create') }}" class="text-sm text-indigo-600 hover:text-indigo-900">New</a>
                    </div>
                            ...
                            @foreach ($events as $event)
                                <div class="rounded-md border border-gray-200 p-3">
                                    ...
                                    <div class="mt-1 text-sm text-gray-700">
                                        @if ($event->project)
                                            <a class="text-indigo-600 hover:text-indigo-900" href="{{ route('projects.show', $event->project) }}">{{ $event->project->name }}</a>
                                        @else
